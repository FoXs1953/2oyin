...
const translations = {
  ru: {
    ...
    heroSubtitle:
      "Мы сдаем в аренду активные, интеллектуальные и национальные игры, которые превращают любое мероприятие в настоящее шоу.",
  },
  kz: {
    ...
    heroSubtitle:
      "Біз кез келген іс-шараны нағыз шоуға айналдыратын белсенді, интеллектуалды және ұлттық ойындарды жалға береміз.",
  },
};
...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "Игры проводит ВАШ ведущий, вы должны уведомить ведущего о правилах игр и сценарии",
  },
  {
...
const LandingPage = () => {
  ...
  const scrollToGame = (gameName) => {
    ...
    const el = document.querySelector(
      `[data-game="${escape(gameName)}"]`
    );

    if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });
    else scrollToSection("games");
  };

  const whatsappNumber = "77779614990";
  const whatsappMessage =
    lang === "ru"
      ? `Здравствуйте! Хочу заказать игры${selectedGame ? ` («${selectedGame}»)` : ""}.`
      : `Сәлеметсіз бе! Ойындарға тапсырыс бергім келеді${
            ...
            <div className="hero-gradient-ring glass-card relative overflow-hidden rounded-3xl p-5 sm:p-6">
              ...
              <div className="relative space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs sm:text-sm">
                  {gamesToRender.map((game) => (
                    <button
                      key={game.name}
                      type="button"
                      onClick={() => scrollToGame(game.name)}
                ...
                <div className="flex items-center justify-between rounded-2xl border border-dashed border-rose-200 bg-white/70 px-3 py-2.5 text-xs text-[var(--text-secondary)]">
                  <span>
                    Продажа «Асыки», «Горшкомания», «Бейби Баттл» и «Арқан тарту».
                  </span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="rounded-full bg-rose-50 px-2.5 py-1 text-[0.65rem] font-semibold uppercase tracking-wide text-rose-500">
                      {["Асыки", "Горшкомания", "Baby Battle", "Арқан тарту"].includes(
                        game.name
                      )
                        ? "Аренда + продажа"
                        : "Аренда"}
          ...
          <div className="grid gap-4 md:grid-cols-[1.3fr,1fr] md:items-center">
            <div className="space-y-3">
              ...
              <p className="text-xs text-rose-50/90 sm:text-sm">
                {lang === "ru"
                  ? "Нажмите кнопку — откроем WhatsApp и сразу обсудим игры. Без ввода номера."
                  : "Түймені басыңыз — WhatsApp ашылып, бірден ойындарды талқылаймыз. Нөмір енгізусіз."}
              </p>
              {selectedGame && (
            ...
            <div className="space-y-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-white/95 px-4 py-3 text-xs font-semibold text-rose-600 shadow-md shadow-red-400/30 transition hover:bg-white sm:text-sm"
              >
                <MessageCircle className="h-4 w-4 text-emerald-500" />
                <span>{lang === "ru" ? "Написать в WhatsApp" : "WhatsApp арқылы жазу"}</span>
              </a>
              <p className="text-[0.65rem] text-rose-100/90 sm:text-[0.7rem]">
                {lang === "ru"
                  ? "Нажмете кнопку — откроется чат WhatsApp с вашим запросом."
                  : "Түймені бассаңыз — WhatsApp-та сұрағыңыз дайын болып ашылады."}
              </p>
            </div>
          </div>
        </section>
